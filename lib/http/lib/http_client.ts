...
export class HTTPClient extends Map<string, RateLimitBucket> {
  ...
  request<T = unknown>(path: string, options?: RequestOptions): Promise<T> {
    ...
    const bucket = this.getRateLimitBucket(path, options?.method);
  ...
  private createRequest(path: string, options?: RequestOptions) {
    ...
    if (options?.query) {
      url += `?${new URLSearchParams(options.query as Record<string, string>)}`;
    }
